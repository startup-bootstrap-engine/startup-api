export const NPC_BASE_HEALTH_MULTIPLIER = 1.75;
export const NPC_SPEED_MULTIPLIER = 0.75;
export const NPC_SKILL_LEVEL_MULTIPLIER = 3.5;
export const NPC_SKILL_STRENGTH_MULTIPLIER = 3.5;
export const NPC_SKILL_DEXTERITY_MULTIPLIER = 2.5;
export const NPC_SKILL_RESISTANCE_MULTIPLIER = 1.5;
// Performance adjustments
export const NPC_MIN_DISTANCE_TO_ACTIVATE = 10; // grid cells

export const NPC_FRIENDLY_NEUTRAL_FREEZE_CHECK_CHANCE = 25;
export const NPC_HOSTILE_FREEZE_CHECK_CHANCE = 25;

// PZ
export const NPC_CAN_ATTACK_IN_NON_PVP_ZONE = true;

// Trader
export const NPC_TRADER_INTERACTION_DISTANCE = 14;

// Giant form
export const NPC_GIANT_FORM_SPAWN_PERCENTAGE_CHANCE = 15;
export const NPC_GIANT_FORM_STATS_MULTIPLIER = 3;
export const NPC_GIANT_FORM_LOOT_MULTIPLIER = 3;
export const NPC_GIANT_FORM_EXPERIENCE_MULTIPLIER = 2.5;

//! DONT TOUCH THIS SHIT, THIS CAN CAUSE SERIOUS PERFORMANCE ISSUES IF INCREASED.
export const NPC_CYCLE_INTERVAL_RATIO = 0.95; // higher means faster cycles

export const NPC_FREEZE_CHECK_INTERVAL = 1000;

// NPC Battle
export const NPC_BATTLE_CYCLE_INTERVAL = 1500;

export const NPC_INACTIVITY_THRESHOLD = 30000; // 30 seconds

export const NPC_MAX_STUN_LEVEL = 50;

export const NPC_BOOST_HEALING_WHEN_DYING_PROBABILITY = 1.5;
