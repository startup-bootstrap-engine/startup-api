...
export class CharacterCreateSocketHandler {
  constructor(private newRelic: NewRelic, private socketSessionControl: SocketSessionControl, private locker: Locker) {}

  public async manageSocketConnections(channel: SocketChannel, character: ICharacter): Promise<void> {
    const canProceed = await this.locker.lock(`manage-socket-connections-${character._id}`);
    ...
    try {
      ...
      await this.locker.unlock(`manage-socket-connections-${character._id}`);
    }
  }
